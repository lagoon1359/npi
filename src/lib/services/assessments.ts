...
export const assessmentService = {
  ...
  async getAssessmentStats(assessment_id: string) {
    try {
      ...
      const submitted_count = gradesData?.filter(g => g.submitted_date).length || 0
      const graded_count = gradesData?.filter(g => g.score !== null).length || 0
  ...
  async bulkUpdateGrades(grades: Array<{
    ...
    try {
      ...
      const student_ids = [...new Set(data?.map(g => g.student_id) || [])]
      await Promise.all(student_ids.map(id => this.triggerGPARecalculation(id)))
  ...
  async getCompletionRate(filters: any) {
    ...
    const graded = allGrades.filter(g => g.score !== null).length
  ...
  async getCoursesWithAssessmentsCount(filters: any) {
    ...
    if (!data) return 0
    const uniqueCourses = new Set(data.map(a => a.course_id))
