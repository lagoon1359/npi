    ...
    const loadStudentData = async () => {
      try {
        ...
        if (transcriptData) {
          ...
          const totalGradePoints = transformedCourseGrades.reduce((sum, course) =>
            sum + (course.grade_points * course.credit_hours), 0)
          const totalCreditHours = transformedCourseGrades.reduce((sum, course) =>
          ...
          setGpaData({
            ...
            total_credits_earned: transformedCourseGrades.filter(c => c.is_passed).reduce((sum, c) => sum + c.credit_hours, 0),
